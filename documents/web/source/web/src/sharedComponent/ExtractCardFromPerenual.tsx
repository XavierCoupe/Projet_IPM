...
function ExtractCardFromPerenual({id} : {id : string}) {
    ...
    useEffect(() => {
      async function fetchData() {
        try {
          const response = await axios.get('https://perenual.com/api/species/details/' + id + '?key=' + GetPerenualApiKey({ num: 0 }));
          setResponseData(response.data);
          const resp = await axios.get('https://perenual.com/api/species-care-guide-list?key=' + GetPerenualApiKey({ num: 0 }) + '&species_id=' + id);
